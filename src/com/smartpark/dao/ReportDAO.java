...
public class ReportDAO {
    ...
    public List<ParkingSession> getParkingHistoryByCustomer(int customerId) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    history.add(new ParkingSession(
                            rs.getInt("id"),
    ...
    public void printCurrentlyParkedVehicles() throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                found = true;
                System.out.printf("Slot: %-8s | Plate: %-12s | Type: %-5s | Entry: %s%n", 
                        rs.getString("slot_number"), rs.getString("license_plate"), 
                        rs.getString("vehicle_type"), rs.getTimestamp("entry_time"));
            }
    ...
    public void printSystemCapacityStatus() throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                System.out.printf("Type: %-5s | Status: %-10s | Count: %d%n", 
                        rs.getString("slot_type"), rs.getString("status"), rs.getInt("count"));
            }
        }
    }
