...
public class ParkingSlotDAO {
    ...
    public ParkingSlot getNearestAvailableSlot(String slotType) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return new ParkingSlot(rs.getInt("id"), rs.getInt("floor_id"), rs.getString("slot_number"), rs.getString("slot_type"), rs.getString("status"));
                }
            }
        }
        return null;
    ...
    public List<ParkingSlot> getAvailableSlots(String slotType) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    slots.add(new ParkingSlot(rs.getInt("id"), rs.getInt("floor_id"), rs.getString("slot_number"), rs.getString("slot_type"), rs.getString("status")));
                }
            }
        }
        return slots;
