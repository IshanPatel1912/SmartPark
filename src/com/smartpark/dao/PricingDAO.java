...
public class PricingDAO {
    ...
    public void addOrUpdatePricing(Pricing pricing) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            stmt.setBigDecimal(2, pricing.getBaseRate());
            stmt.setBigDecimal(3, pricing.getHourlyRate());
            stmt.executeUpdate();
        }
    }
    ...
    public Pricing getPricingByVehicleType(String vehicleType) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return new Pricing(rs.getInt("id"), rs.getString("vehicle_type"), rs.getBigDecimal("base_rate"), rs.getBigDecimal("hourly_rate"));
                }
            }
        }
        return null;
